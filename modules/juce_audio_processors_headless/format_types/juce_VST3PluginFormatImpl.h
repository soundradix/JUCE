...
//==============================================================================
class VST3PluginInstanceHeadless;
                                 ...
                                 private ComponentRestarter::Listener
{
    ...
    ~VST3HostContextHeadless() override = default;
    ...
    //==============================================================================
    inline tresult PLUGIN_API beginEdit (Vst::ParamID paramID) override;
    inline tresult PLUGIN_API performEdit (Vst::ParamID paramID, Vst::ParamValue valueNormalized) override;
    inline tresult PLUGIN_API endEdit (Vst::ParamID paramID) override;

    inline tresult PLUGIN_API restartComponent (Steinberg::int32 flags) override;
    ...
    void setPlugin (VST3PluginInstanceHeadless* instance)
    {
...
private:
    ...
    inline void restartComponentOnMessageThread (int32 flags) override;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VST3HostContextHeadless)
};
...
{
    VST3ComponentHolder (const VST3ModuleHandle& m, VSTComSmartPtr<VST3HostContextHeadless> h)
        : module (m), host (h)
    {
    }
...
public:
    ...
    {
        VST3Parameter (VST3PluginInstanceHeadless& parent, Steinberg::int32 vstParameterIndex)
    ...
    private:
        ...
        VST3PluginInstanceHeadless& pluginInstance;
        const Steinberg::int32 vstParamIndex;
        Vst::ParameterInfo cachedInfo = fetchParameterInfo();
    };

    //==============================================================================
    explicit VST3PluginInstanceHeadless (std::unique_ptr<VST3ComponentHolder> componentHolder)
...
private:
    ...
    */
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VST3PluginInstanceHeadless)
};

JUCE_END_IGNORE_DEPRECATION_WARNINGS

//==============================================================================
tresult VST3HostContextHeadless::beginEdit (Vst::ParamID paramID)
{
    if (plugin == nullptr)
        return kResultTrue;
...
tresult VST3HostContextHeadless::performEdit (Vst::ParamID paramID, Vst::ParamValue valueNormalised)
{
    if (plugin == nullptr)
        return kResultTrue;
...
tresult VST3HostContextHeadless::endEdit (Vst::ParamID paramID)
{
    if (plugin == nullptr)
        return kResultTrue;
...
tresult PLUGIN_API VST3HostContextHeadless::setDirty (TBool needsSave)
{
    if (needsSave)
...
void VST3HostContextHeadless::restartComponentOnMessageThread (int32 flags)
{
    if (plugin == nullptr)
    {
        jassertfalse;
...
//==============================================================================
tresult VST3HostContextHeadless::notifyProgramListChange (Vst::ProgramListID, Steinberg::int32)
{
    if (plugin != nullptr)
...
//==============================================================================
static inline void recursiveFileSearch (VST3PluginFormatHeadless& format, StringArray& results, const File& directory, const bool recursive)
{
    for (const auto& iter : RangedDirectoryIterator (directory, false, "*", File::findFilesAndDirectories))
    {
        ...
        if (recursive && (! isPlugin) && f.isDirectory())
            recursiveFileSearch (format, results, f, true);
    }
}

static StringArray getLibraryPaths ([[maybe_unused]] VST3PluginFormatHeadless& format, const String& fileOrIdentifier)
{
   #if JUCE_WINDOWS
    if (! File (fileOrIdentifier).existsAsFile())
    {
        StringArray files;
        recursiveFileSearch (format, files, fileOrIdentifier, true);
        return files;
    }
   #endif
...
static std::unique_ptr<AudioPluginInstance> createVST3Instance (VST3PluginFormatHeadless& format,
                                                                ...
                                                                VSTComSmartPtr<VST3HostContextHeadless> host)
{
    ...
    auto holder = std::make_unique<VST3ComponentHolder> (module, host);

    if (! holder->initialise())
        return nullptr;

    auto instance = std::make_unique<TypeToCreate> (std::move (holder));
...
template <typename TypeToCreate>
static void createVst3InstanceImpl (VST3PluginFormatHeadless& format,
                                    VSTComSmartPtr<VST3HostContextHeadless> host,
                                    const PluginDescription& description,
                                    AudioPluginFormat::PluginCreationCallback callback)
{
    for (const auto& file : getLibraryPaths (format, description.fileOrIdentifier))
    {
        if (auto result = createVST3Instance<TypeToCreate> (format, description, file, host))
        {
